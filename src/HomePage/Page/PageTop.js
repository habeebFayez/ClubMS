...
const PageTop = ({user,club,isActive,isModalOpenForCreatEvent,isClubPage,allClubsCategories,allEventsCategories ,allClubs,isSearchvis,categories ,events,title}) => {
    ...
    const handleFilterClick = (filter) => {
        ...
        switch (filter) {
            case 'all' : setFilterResults(events,allClubs)
                break;
            case 'club'  : setFilterResults()
                break;
            case 'event' : setFilterResults()
                break;
    ...
    return (
        ...
        <div className={!isSearchvis?"pageUp-nav":"pageUp-nav-title-only"} onClick={closeDropdown}>
            ...
            {inputValue && activeFilter === 'club' && (
                <div className={isClubPage ? "search-results-content-club-page" : "search-results-content"}>
                    {allClubs.filter(input => {
                        ...
                        <div className="SidePage-img-holder" onClick={() => navigate(`/clubprofile`, { state: { club: result } })}>
                            <img className="SidePage-img" src={result.clubProfilePicURL ? result.clubProfilePicURL : ""} alt={result.clubName} />
                            <div>
            ...
            {inputValue && activeFilter === 'all' && (
                <div className={isClubPage ? "search-results-content-club-page-inhalf" : "search-results-content-inhalf"}>
            ...
            {inputValue && activeFilter === 'event' &&
                <div className={isClubPage ? "search-results-content-club-page" : "search-results-content"}>
                    {events.filter(input => {
                        ...
                        <div className="SidePage-img-holder" onClick={() => navigate("/event", {state: {from: result}})}>
                        <img className="SidePage-img" src={result.eventPostMediaURL ? result.eventPostMediaURL : ""}/>
                        <div>
                            <h5>{result.eventName ? result.eventName.slice(0, 29).toUpperCase() : ""}</h5>
                            <p style={{fontSize: 11}}> Date: {result.eventStartingDate} Time : {result.eventNote}</p>
                        </div>
